...
{
public:
  vtkTypeRevisionMacro(vtkPointSetOutlierRemoval, vtkPolyDataAlgorithm);
      ...
      bool operator<(NumberedItem<T> NI1, NumberedItem<T> NI2)
{
  ...
  template <typename T>
      vtkstd::vector<unsigned int> ParallelSortIndices(const std::vector<T> &Things, bool descending = false)
{
    //this function returns the new order of the indices after the sort
  vtkstd::vector<NumberedItem<T> > Pairs(Things.size());
  ...
  vtkstd::sort(Pairs.begin(), Pairs.end());
    
  vtkstd::vector<unsigned int> SortedIndices(Things.size());
